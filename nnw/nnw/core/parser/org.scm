...
(define* (paragraph-node->input n tags)
...
(define (edit-block-drawer cur prev)
  (if (null? prev)
      prev
	...
	(cons
	 (sxml-match block
	   ((block (@ (tags ,tags) . ,meta) . ,chd)
	    `(block (@ (tags ,(merge-tags tags (if tgs (string-split tgs #\space) '()))) ,@meta
		       ,@(map k-pair->k-list (delete (assq 'tags metadata) metadata))) ,@children ,@chd))
	   ((block (@ . ,meta) . ,chd)
	    `(block (@ ,@meta ,@(map k-pair->k-list metadata)) ,@children ,@chd))
	   ((block . ,chd)
...
(define (edit-prev cur prev)
  (cond ((block-drawer? cur) (edit-block-drawer cur prev))
	(else prev)))

(define* (fold+convert lst #:key (tags '()))
  (define (convert+cons elem prev)
    (let ((result (node->input elem tags))
